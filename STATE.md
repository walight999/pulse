...
| Date | Update |
|---|---|
| 2026-05-15 | **v1.8 close-the-infra-gaps** shipped (e6ea746). Cloud FastAPI server bootable via `python -m api.server --dev` with real HS256 JWT verify path (PyJWT or hand-rolled HMAC). PWA service worker on landing — caches /demo / docs / methodology / etc., new /offline fallback. ChatGPT Plus export ZIP uploader in Settings → Integrations (wires existing parse_export_archive). New `.github/workflows/build-macos.yml` produces unsigned `.app` artifact on every push; release.yml macOS job ungated. New `CLOUD_DEPLOY.md` with Supabase + Fly.io step-by-step. pytest 51 → **60** (9 new API tests). |
| 2026-05-15 | **v1.7 stop-the-scaffolding** shipped (b7b6288). OpenAI `/v1/usage` parser, Cursor IDE local state-DB parser, GitHub Copilot org-level sync, Gemini key validation, and Ask pulse assistant (Anthropic Messages API + 5 real SQL tool functions) all wired in. Previously these were `NotImplementedError` stubs or written-but-not-wired code from v1.1. Integration matrix on `/` flipped from 1 Available / 7 Coming → 7 Available / 1 Q3 / 2 Planned. pytest 51/51 still pass. |
| 2026-05-15 | Landing audit Phases 1-6 shipped (20 routes static, pytest 51/51). Phase 1 (d174044) honesty: status badges, lifetime defused, governing law. Phase 2 (a2fef41) activation: `/download`, `/methodology`, segmented waitlist. Phase 3 (75ec298) credibility: personas, integrations matrix, self-host comparison, `/roadmap`. Phase 4 (26774d7) plumbing: pluggable `/api/waitlist` (Supabase + Resend opt-in), onboarding consent, `tracker.py` honors privacy toggles. Phase 5 (6420bfc) polish: trust strip, `/docs` hub, CSS overflow safety, a11y, Vercel Analytics, UTM capture, Finance/Ops persona, activity allowlist + danger-zone wipe, onboarding data location + demo-data seed. Phase 6 (a2eb35f) finish-the-partials: `/demo` interactive with persona toggle + computed metrics + sample CSV, `/alternatives` 800-word honest comparison hub, `/compare/claude-code-cost-tracker` + `/compare/cursor-cost-tracker` focused SEO landings. 23 of 25 brief sections done; only deferred item is #22 admin dashboard. |
